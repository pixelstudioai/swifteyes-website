...
if(document.querySelector(".hero.overhero")){ var nv=document.getElementById("nav"); if(nv) nv.classList.add("glass"); }
...
document.addEventListener("play", function(e){ if(e.target && e.target.tagName === "VIDEO"){ document.querySelectorAll("video").forEach(function(v){ if(v !== e.target) v.pause(); }); } }, true);
document.querySelectorAll(".explorer").forEach(function(ex){
  var tabs = [].slice.call(ex.querySelectorAll(".exptab")), panes = [].slice.call(ex.querySelectorAll(".exppane"));
  tabs.forEach(function(t, i){
    ...
    t.addEventListener("keydown", function(e){
      ...
      if(!n) return;
      e.preventDefault();
      var nx = tabs[(i + n + tabs.length) % tabs.length];
      nx.click(); nx.focus();
    });
  });
});
